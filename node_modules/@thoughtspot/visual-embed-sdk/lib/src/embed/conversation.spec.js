...
describe('ConversationEmbed', () => {
    ...
    describe('spotter sidebar config params', () => {
        it.each([
            ...
            const conversationEmbed = new SpotterEmbed(getRootEl(), viewConfig);
            await conversationEmbed.render();
            expectUrlMatchesWithParams(getIFrameSrc(), `http://${thoughtSpotHost}/v2/?${defaultParams}&isSpotterExperienceEnabled=true&${expectedParam}#/embed/insights/conv-assist?worksheet=worksheetId&query=`);
        });
        it.each([
            ...
            conversationEmbed.handleError = jest.fn();
            await conversationEmbed.render();
            expect(conversationEmbed.handleError).toHaveBeenCalledWith(expect.objectContaining({
                errorType: ErrorDetailsTypes.VALIDATION_ERROR,
        ...
        it('should render with multiple spotterSidebarConfig options', async () => {
            ...
            const conversationEmbed = new SpotterEmbed(getRootEl(), viewConfig);
            await conversationEmbed.render();
            expectUrlMatchesWithParams(getIFrameSrc(), `http://${thoughtSpotHost}/v2/?${defaultParams}&isSpotterExperienceEnabled=true&enablePastConversationsSidebar=true&spotterSidebarDefaultExpanded=true&spotterSidebarTitle=Chats&spotterConversationsBatchSize=25&spotterNewChatButtonTitle=New#/embed/insights/conv-assist?worksheet=worksheetId&query=`);
        });
    });
});
